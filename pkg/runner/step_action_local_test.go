...
func (salm *stepActionLocalMocks) runAction(step actionStep, actionDir string, actionRef *actionRef) common.Executor {
	args := salm.Called(step, actionDir, actionRef)
...
func TestStepActionLocalTest(t *testing.T) {
	...
	salm.On("runAction", sal, filepath.Clean("/tmp/path/to/action"), (*actionRef)(nil)).Return(func(_ context.Context) error {
		return nil
	})
